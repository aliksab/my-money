...
router
  ...
  .get(auth, async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .send(list.filter((manipulation) => manipulation.userId == userId));
    } catch (e) {
      res.status(500).json({
...
router
  ...
  .delete(auth, async (req, res) => {
    try {
      ...
      const removedManipulation = await InvoiceManipulation.findByIdAndDelete(
        manipulationId
      );
  ...
  .patch(auth, async (req, res) => {
    try {
      ...
      if (manipulationId) {
        const updatedInvoiceManipulation =
          await InvoiceManipulation.findByIdAndUpdate(
            manipulationId,
            req.body,
            { new: true }
          );
